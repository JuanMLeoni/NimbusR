...
test_that("Falla si destfile es vacío o inválido", {
  expect_error(getNH0098(""), "El parametro 'destfile' debe ser una cadena de texto no vacia")
  expect_error(getNH0098(NULL), "El parametro 'destfile' debe ser una cadena de texto no vacia")
  expect_error(getNH0098(123), "El parametro 'destfile' debe ser una cadena de texto no vacia")
})
...
test_that("Falla si la URL no es accesible", {
  ...
  with_mocked_bindings(
    ...
    expect_error(getNH0098(tempfile()), "Error en la descarga del archivo desde la URL proporcionada")
  )
})
...
test_that("Falla si el archivo no es un CSV válido", {
  ...
  with_mocked_bindings(
    ...
    {
      expect_error(getNH0098(archivo_temp),
